...
export const ClubCard = ({ club, onClick, className }: ClubCardProps) => {
  ...
  return (
    <Card 
      onClick={onClick}
      className={cn(
        "relative overflow-hidden border-0 rounded-[1.25rem] transition-all duration-500 shadow-2xl cursor-pointer group select-none",
        "bg-gradient-to-br",
        color,
        className
      )}
    ...
    >
        ...
        <div className="relative z-10 flex flex-col h-full justify-between p-1">
          ...
          <CardHeader className="space-y-2 pt-4 px-4 md:pt-5 md:px-5">
            {/* Floating Icon Badge */}
            ...
            <div className="space-y-1 pt-1 md:pt-2">
              ...
              <p className="text-[9px] font-bold text-neutral-900/70 tracking-widest uppercase font-sans pl-1 border-l-2 border-neutral-900/30 ml-1">
                {tagline}
              </p>
            </div>
          </CardHeader>
          ...
          <CardContent className="pb-2 px-4 md:px-5">
             <CardDescription className="text-neutral-900/90 font-medium text-xs leading-relaxed line-clamp-3 mix-blend-multiply">
                {description}
             </CardDescription>
          </CardContent>
          ...
          <CardFooter className="pb-4 pt-1 px-4 md:pb-5 md:px-5">
            <div className="flex items-center gap-4 group/btn w-full">
               {/* Expanding Button: Full width on mobile, expands on hover on desktop */}
               <span className="h-8 rounded-full bg-neutral-900 text-white flex items-center justify-between px-1 w-full md:w-8 md:group-hover/btn:w-full transition-all duration-300 overflow-hidden shadow-xl hover:bg-black">
                  <span className="whitespace-nowrap text-[9px] font-bold pl-3 opacity-100 md:absolute md:opacity-0 md:group-hover/btn:opacity-100 transition-opacity duration-300 delay-75 uppercase tracking-wider">
                    Explore
                  </span>
                  <span className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center ml-auto shrink-0">
...
export const ClubCardSkeleton = ({ className }: { className?: string }) => {
  return (
    <Card 
      className={cn(
        "relative overflow-hidden border border-neutral-200 dark:border-white/10 rounded-[1.25rem] bg-neutral-100 dark:bg-neutral-900 shadow-xl h-full flex flex-col justify-between p-1",
        className
      )}
    >
      {/* Background Texture */}
      ...
      {/* Header */}
      <CardHeader className="space-y-3 pt-5 px-4 md:pt-6 md:px-5 relative z-10">
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-neutral-200 dark:bg-neutral-800/50 animate-pulse border border-neutral-300 dark:border-neutral-700/50" />
        <div className="space-y-2 pt-2">
          <div className="h-5 md:h-6 w-3/4 bg-neutral-200 dark:bg-neutral-800/50 rounded-lg animate-pulse" />
          <div className="h-3 w-1/3 bg-neutral-200 dark:bg-neutral-800/50 rounded-md animate-pulse" />
        </div>
      </CardHeader>

      {/* Content */}
      <CardContent className="px-4 md:px-5 pb-2 relative z-10">
      ...
      {/* Footer */}
      <CardFooter className="pb-4 px-4 md:pb-5 md:px-5 relative z-10">
          <div className="flex items-center gap-4 w-full">
               <div className="h-8 w-full rounded-full bg-neutral-200 dark:bg-neutral-800/50 animate-pulse" />
          </div>
      </CardFooter>
