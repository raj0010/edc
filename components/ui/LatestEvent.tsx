...
export const LatestEvent: React.FC<LatestEventProps> = ({ club, onRegister }) => {
  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-4 py-12 md:py-20 relative z-20">
      <motion.div 
      ...
      >
        ...
        {/* Main Content (Left) */}
        <div className="flex-1 p-6 md:p-10 relative z-20 border-b md:border-b-0 md:border-r border-white/10 border-dashed">
            <div className="flex justify-between items-start mb-6 md:mb-8">
                <div className="flex flex-col">
                   <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 mb-2">Upcoming Event</span>
            ...
            <h2 className="text-3xl md:text-5xl font-black text-neutral-900 dark:text-white font-display uppercase leading-[0.9] mb-6 tracking-tighter">
                {event.title}
            </h2>
            ...
            <div className="flex items-center gap-4">
                <button 
                    onClick={onRegister}
                    className="px-8 py-3 bg-neutral-900 dark:bg-white text-white dark:text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:scale-105 transition-transform shadow-lg shadow-black/10 dark:shadow-white/10 w-full md:w-auto"
                >
                    Get Ticket
                </button>
            </div>
        ...
        {/* Stub (Right) */}
        <div className="w-full md:w-64 bg-neutral-100/50 dark:bg-black/20 p-6 md:p-8 relative z-20 flex flex-row md:flex-col justify-between items-center text-center">
             ...
             <div className="absolute -bottom-3 left-1/2 md:top-auto md:bottom-auto md:top-1/2 md:-right-3 w-0 h-0 md:w-6 md:h-6 bg-transparent rounded-full"></div> 
             
             <div>
                <div className="text-sm font-bold text-neutral-400 uppercase tracking-widest mb-1 md:mb-2">{month}</div>
                <div className="text-4xl md:text-6xl font-black text-neutral-900 dark:text-white font-display tracking-tighter">{day}</div>
             </div>

             <div className="hidden md:block w-full py-4 border-t border-b border-white/10 my-4">
                 <QrCode className="w-20 h-20 mx-auto text-neutral-800 dark:text-white opacity-90" />
             </div>

             <div className="flex flex-col items-center gap-1 opacity-50">
                <Barcode className="w-24 md:w-32 h-8 text-neutral-800 dark:text-white" />
                <span className="text-[8px] font-mono text-neutral-500 tracking-[0.2em] hidden md:block">ADMIT ONE</span>
             </div>
        </div>
      </motion.div>
    </section>
